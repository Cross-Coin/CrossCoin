...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("crosscoin"))
        return false;
...
{
    // Convert crosscoin:// to crosscoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("crosscoin://"))
    {
        uri.replace(0, 12, "crosscoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "CrossCoin.lnk";
}
...
{
    return GetAutostartDir() / "crosscoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("CrossCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  crosscoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("CrossCoin-Qt"));
